...
static void location_decode(struct golioth_location_req *location_req,
                            const uint8_t *payload,
                            size_t payload_size)
{
    struct golioth_location_rsp *location_rsp = location_req->rsp;
    ZCBOR_STATE_D(zsd, 1, payload, payload_size, 1, 0);
    struct zcbor_map_entry map_entries[] = {
        ZCBOR_TSTR_LIT_MAP_ENTRY("lat", zcbor_map_int64_decode, &location_rsp->latitude),
        ZCBOR_TSTR_LIT_MAP_ENTRY("lon", zcbor_map_int64_decode, &location_rsp->longitude),
        ZCBOR_TSTR_LIT_MAP_ENTRY("acc", zcbor_map_int64_decode, &location_rsp->accuracy),
    };
    int err;
    ...
    if (err)
    {
        ...
        location_req->status = GOLIOTH_ERR_INVALID_FORMAT;
        return;
    }
}
...
static void location_cb(struct golioth_client *client,
                        ...
                        void *arg)
{
    struct golioth_location_req *location_req = arg;

    location_req->status = status;
    ...
    if (status != GOLIOTH_OK)
    {
        if (status == GOLIOTH_ERR_COAP_RESPONSE && payload_size == 0)
        {
            ...
            location_req->status = GOLIOTH_ERR_NULL;
        }
        else
        {
    ...
    if (location_req->rsp)
    {
        location_decode(location_req, payload, payload_size);
    }
}
