...
static void test_request_dropped_if_client_not_running(void) {
    ...
    TEST_ASSERT_EQUAL(GOLIOTH_ERR_INVALID_STATE,
                      golioth_lightdb_set_int_async(_client, "a", 1, NULL, NULL));
    TEST_ASSERT_EQUAL(
        GOLIOTH_ERR_INVALID_STATE,
        golioth_lightdb_get_async(_client, "a", GOLIOTH_CONTENT_TYPE_JSON, NULL, NULL));
    TEST_ASSERT_EQUAL(GOLIOTH_ERR_INVALID_STATE,
                      golioth_lightdb_delete_async(_client, "a", NULL, NULL));
    TEST_ASSERT_EQUAL(GOLIOTH_ERR_INVALID_STATE,
                      golioth_lightdb_observe_async(_client, "a", NULL, NULL));
...
static void test_lightdb_set_get_sync(void) {
    ...
    TEST_ASSERT_EQUAL(
        GOLIOTH_OK,
        golioth_lightdb_get_int_sync(_client, "test_int", &get_randint, TEST_RESPONSE_TIMEOUT_S));
    TEST_ASSERT_EQUAL(randint, get_randint);
}

static bool _on_get_test_int2_called = false;
static int32_t _test_int2_value = 0;
static void on_get_test_int2(struct golioth_client *client,
...
static void test_lightdb_set_get_async(void) {
    ...
    TEST_ASSERT_EQUAL(GOLIOTH_OK,
                      golioth_lightdb_set_int_async(_client,
                                                    "test_int2",
                                                    randint,
                                                    on_set_test_int2,
                                                    &set_async_response));
    ...
    TEST_ASSERT_EQUAL(GOLIOTH_OK,
                      golioth_lightdb_get_async(_client,
                                                "test_int2",
...
static void on_test_timeout(struct golioth_client *client,
                            const struct golioth_response *response,
                            const char *path,
                            const uint8_t *payload,
                            size_t payload_size,
...
static void test_request_timeout_if_packets_dropped(void) {
    ...
    int32_t dummy = 0;
    TEST_ASSERT_EQUAL(GOLIOTH_ERR_TIMEOUT,
                      golioth_lightdb_get_int_sync(_client, "expect_timeout", &dummy, 1));
    TEST_ASSERT_EQUAL(GOLIOTH_ERR_TIMEOUT,
                      golioth_lightdb_set_int_sync(_client, "expect_timeout", 4, 1));
    TEST_ASSERT_EQUAL(GOLIOTH_ERR_TIMEOUT,
                      golioth_lightdb_delete_sync(_client, "expect_timeout", 1));
    ...
    TEST_ASSERT_TRUE((xTaskGetTickCount() * portTICK_PERIOD_MS) - now
                     > (1000 * CONFIG_GOLIOTH_COAP_RESPONSE_TIMEOUT_S));
...
static void on_test_int3(struct golioth_client *client,
                         const struct golioth_response *response,
                         const char *path,
                         const uint8_t *payload,
                         size_t payload_size,
