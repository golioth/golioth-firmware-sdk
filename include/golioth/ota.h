...
/// Verify the component sha256 hash supplied by server matches on calculated locally
///
/// This function compares the component hash received in the OTA manifest to one that is supplied.
/// Use the golioth_sys_sha256_*() functions (or similar) to calculate the has as blocks are
...
/// server.
/// @retval GOLIOTH_ERR_INVALID_FORMAT The \p local_hash does not match the hash the server
/// supplied.
