...
def verify_dfu_status_logs(logs):
    ...
    for i, r in enumerate(golioth_ota_reason):
        assert int(logs[GOLIOTH_OTA_REASON_CNT - i].metadata['update']['lobster']['reasonCode']) == i
        assert int(logs[GOLIOTH_OTA_REASON_CNT - i].metadata['update']['lobster']['stateCode']) == i % GOLIOTH_OTA_STATE_CNT
        assert logs[GOLIOTH_OTA_REASON_CNT - i].metadata['update']['lobster']['state'] == golioth_ota_state[ i % GOLIOTH_OTA_STATE_CNT ]
        assert logs[GOLIOTH_OTA_REASON_CNT - i].metadata['update']['lobster']['package'] == "lobster"
        assert logs[GOLIOTH_OTA_REASON_CNT - i].metadata['update']['lobster']['version'] == "2.3.4"
        assert logs[GOLIOTH_OTA_REASON_CNT - i].metadata['update']['lobster']['target'] == "5.6.7"
...
async def test_reason_and_state(board, device, project, artifacts, cohort):
    ...
    await board.wait_for_regex_in_line(f"golioth_ota_get_state: {(GOLIOTH_OTA_REASON_CNT - 1) %
                                       ...
                                       timeout_s=GOLIOTH_OTA_REASON_CNT)
