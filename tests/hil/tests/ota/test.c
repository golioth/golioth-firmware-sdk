                                            ...
                                            void *arg)
{
    ...
    if (block_idx == 0)
    {
        /* This was already done with the create() function
         * but let's call it again to test the api
         */
        golioth_sys_sha256_init(ctx->sha);
    }
    ...
    if (is_last)
    {
        ...
        golioth_sys_hex2bin(component->hash,
                            strlen(component->hash),
                            sha_server,
                            sizeof(sha_server));
...
static void test_resume(struct golioth_ota_component *walrus)
{
    ...
    while (counter < 10)
    {
        ...
        else
        {
            ...
            golioth_sys_sha256_free(ctx.sha);
            ctx.sha = NULL;
            return;
        }
                        ...
                        void *arg)
{
    ...
    if (main == NULL)
    {
        const struct golioth_ota_component *walrus = golioth_ota_find_component(&manifest, "walrus");
        if (!walrus)
        {
